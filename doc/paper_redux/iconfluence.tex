\section{\Iconfluence{}}
A \defword{distributed object} is a triple $(S, s_0, \join)$ where $S$ is a set
of states, $s_0 \in S$ is a designated start state, and $\join: S \times S \to
S$ is a binary merge operator that we call join. A \defword{transaction} $t: S
\to S$ is a function that maps one state to another.  An \defword{invariant}
...
We say an expression $e$ \defword{satisfies $I$}, denoted $I(e)$, if
$I(eval(e))$. We say an expression \defword{recursively satisfies $I$}, denoted
...
We say $T$ is \defword{\Iconfluent}, if for all expressions $e_1$ and $e_2$,
