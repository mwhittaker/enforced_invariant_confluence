...
every $T_i$ is a subset of $T$. $O$ is \defword{segmented invariant-confluent}
...
\begin{algorithm}[t]
  \caption{Transaction execution in segmented invariant-confluence system model}%
...
invoke \algoref{InteractiveDecisionProcedure} $|I_i|$ times fore every segment
...
\defword{$\TIcoreachable{s_1, s_2}$}, if there exists some state $s_0 \in I$
...
\todo{Forward reference examples in the evaluation section.}
