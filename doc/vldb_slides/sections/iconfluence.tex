\begin{frame}
  ...
  \note{%
    ...
    For example, the set of natural numbers of joined by max is a distributed
    object. And the powerset of the natural numbers joined by set union is a
    distributed object.
    ...
    something like that, but here we're not making any of those assumptions.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    Next, we define a transaction as a function that maps one state to another.
    For example, the function $t$ that maps an integer $x$ to $x + 1$ is a
    transaction.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    Finally, we say an invariant is a subset of the set of states $S$. For
    example, if our set of states is the set of integers, then the set of
    non-negative integers is an invariant. Notationally, we say $I(s)$ to mean
    $s$ is in the invariant or $s$ satisfies the invariant. And similarly, we
    say $\lnot I(s)$ to mean $s$ does not satisfy the invariant.
...
\begin{frame}
  ...
  \note{%
    ...
    We'll consider the scenario in which we replica our distributed object
    ...
    execute transactions if they maintain the invariant.

    Here, we see that replica $p_2$ and $p_3$ both execute transaction $u$
    leading to state $s_2$.
    ...
    can abort transactions, but they cannot abort a merge.

    And here, we see that the replicas continue to execute transactions and
    merge their states with one another. Informally, a distributed object is
    invariant confluent with respect to a start state, a set of transactions,
    and an invariant if the replicas are guaranteed to never end up in a state
...
\begin{frame}
  ...
  \note{%
    As an example, consider the set of integers merged by max. We have a single
    transaction that increments numbers, and our invariant is that the number
    is non-negative. If our start state is $42$, what is the set of reachable
    states?

    Does anyone in the audience know?

    That's right, the set of all integers greater than or equal to 42 can be
    reached from 42 by incrementing and taking maxes.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    With all these definitions in hand, we can formalize invariant confluence
    very neatly. An object is invariant confluent with respect to some start
    state $s_0$, set of transactions $T$, and invariant $I$ which I'll
    abbreviate to \sTIconfluent{} if the set of reachable states is a subset
    of the invariant.
...
\begin{frame}
  ...
  \note{%
    To help the definition seep into our bones, let's take a look at an
    example. Consider the distributed object conisting of pairs of integers
    $(x, y)$ merged pairwise by max. We consider a transaction $t_x$ that
    increments $x$ part of a pair, and a transaction $t_y$ that decrements the
    $y$ part. Our invariant is that the product of $x$ and $y$ is non-positive.
    Our start state is the pair $(0, 0)$. Is $O$ invariant confluent with
    respect to this start state, set of transactions, and invariant?
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    Well, it's a little bit abstract and hard to think about. We can make the
    example more concrete if we think about it geometrically. Every state is a
    pair of integers, which we can think of as a point on a plane.
...
\begin{frame}
  ...
  \note{%
    ...
    Our state is the origin, so we start at the origin. We can walk right using
    $t_x$, we can walk down using $t_y$, and we can merge two points together.
    Thus, the set of reachable points is the entire fourth quadrant. We can
    reach every point in the fourth quadrant by walking right, down, and
    merging.

    Now, we see the set of points that satisfy the invariant and the set of
    points that reachable, and the set of reachable points is a subset of the
    invariant satisfying points. Thus, our object is invariant confluent. We
    can only reach points that satisfy the invariant. We can never reach a
