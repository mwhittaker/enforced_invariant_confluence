...
\begin{frame}
  ...
  \note{%
    ...
    \sTIconfluent{} if the set of reachable states is a subset of the
    invariant.
...
\begin{frame}
  ...
  \note{%
    Let's take a look at another example. Consider the distributed object
    conisting of pairs of integers $(x, y)$ merged pairwise by max. We consider
    a transaction $t_x$ that increments $x$ part of a pair, and a transaction
...
\begin{frame}
  ...
  \note{%
    Well, it's a little bit abstract and hard to think about, but we can make
    the example more concrete if we think about it geometrically. Every state
    is a pair of integers, which we can think of as a point on a plane like this.
