...
\begin{frame}
  ...
  \note{%
    The gist of it though is that our decision procedure maintains a set of
    reachable states $R$, a set of non reachable states $NR$, and the set $I -
    NR$.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    ...
    by random search and by asking the user.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    Eventually, all the non reachable states are pruned from the invariant and
    the algorithm and invariant closure and invariant confluence are identical.
    At this point, the algorithm checks for invariant closure and terminates.
  }
\end{frame}
