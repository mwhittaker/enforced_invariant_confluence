...
\begin{frame}
  \note{%
    ...
    I'd like to begin my talk, if you don't mind, with a short fairy tale. The
    fairy tale involves an application developer who has some data they need to
    replicate. Unfortunately, the application developer cannot decide whether
    to replicate with weak consistency or strong consistency. The application
    developer stays up all night trying to decide but eventually falls asleep
    at their desk. In their sleep, they are visited by four wise men.
...
\begin{frame}
  ...
  \note{%
    ...
    duper fast, and for many applications, weak consistency is good enough. But
...
\begin{frame}
  ...
  \note{%
    ...
    You see, Hoare encourages us not to settle for weak consistency because a
    weakly consistent system is very hard to reason about.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    ...
    Leslie Lamport encourages us to choose strong consistency. Strongly
...
\begin{frame}
  ...
  \note{%
    But then out teetered Peter, with swagger demeanor.
    ``Why not have both?'' quoth Peter.
    The end.

    Peter Bailis comes to save the day and reminds us that perhaps we can have
    the best of both worlds. In 2014, Peter and some other smart folks
    including Alan defined invariant confluence as a condition under which we
    can replicate a piece of data without the need for strongly consistent
