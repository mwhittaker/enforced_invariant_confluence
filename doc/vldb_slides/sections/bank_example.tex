...
\begin{frame}{Replicated Bank Account}
  ...
  \note{%
    ...
    account balance should never be negative. My balance must be non-negative
    at all times.
  }
\end{frame}
...
\begin{frame}{Coordinating}
  ...
  \note{%
    ...
    deposit 20 dollars. Both transactions succeed.
  }
\end{frame}
...
\begin{frame}{Coordinating}
  ...
  \note{%
    ...
    dollars and 40 dollars. Again, the two replicas coordinate and agree an a
    total order in which to execute transactions.

    They first withdraw 30 dollars, bringing my bank account balance to 20
    dollars. They then try to withdraw of 40 dollars but abort the transaction
    because I only have 40 dollars left. And while the transaction failed,
    you'll note that we never violated our application invariant.
  }
\end{frame}
...
\begin{frame}{Avoiding Coordination}
  ...
  \note{%
    ...
    each other in on the transactions they missed. Notice that even though we
    didn't perform any coordination, the two replicas never violate my critical
    invariant of a non-negative balance.
  }
\end{frame}
