...
\begin{frame}{Replicated Bank Account}
  ...
  \note{%
    So, what is invariant confluence? Well, let's begin with an example.
    ...
    account balance should be non-negative at all times. I'm not allowed to go
    in the hole.
  }
\end{frame}
...
\begin{frame}{Avoiding Coordination}
  ...
  \note{%
    Now, instead of implementing our bank with strong consistency, we could
    instead implement it with weak consistency and avoid coordination
    altogether.
...
\begin{frame}
  ...
  \note{%
    Now, these are just two examples, but if you think about it long enough,
    they are representative of a trend. We can implement deposit transactions
    without any coordination and we're guaranteed to never violate the
    invariant. Withdrawal transactions, on the other hand, do require
    coordination to avoid violating the invariant.

    At a very high level, this is exactly the notion that invariant confluence
    captures. We'll see that deposit transactions are invariant confluent and
    thus do not require coordination while withdrawal transactions are not
    invariant confluent and thus do require coordination.
  }
\end{frame}
