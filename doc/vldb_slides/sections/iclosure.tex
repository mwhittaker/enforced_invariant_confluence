...
\begin{frame}
  ...
  \note{%
    ...
    is hard. I'm not saying it's impossible, but certainly I don't know how to
    do it.
...
\begin{frame}
  ...
  \note{%
    First, a quick refresher on terminology. We say a set $S$ is closed under a
    binary operator $f$ if for every $x$ and $y$ in $S$, $f(x, y)$ is also in
    $S$. For example, even numbers are closed under addition because the sum of
    any two even numbers is even. But odd numbers are not closed under addition
...
\begin{frame}
  ...
  \note{%
    ...
    is that? Well, transactions already preserve invariants. So if merging does
    as well, then every reachable state always maintains the invariant.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    ...
    automatically. for example, remember that example we had with pairs of
...
\begin{frame}
  ...
  \note{%
    So to decide whether an object is invariant confluent, we can take the
...
\begin{frame}
  ...
  \note{%
    ...
    this example, our object is invariant confluent.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    ...
    $(2, -1)$ both satisfy the invariant, but their merge does not.
  }
\end{frame}
...
\begin{frame}
  ...
  \note{%
    This example is actually representative of the overall trend. An invariant
    confluent object may not be invariant closed but only because we're merging
    points that are unreachable. If the invariant is a subset of the reachable
    states, then invariant closure and invariant confluence are identical.

    Why? Well, the forward direction here is what we proved before. For the
    backwards direction, we have that the invariant and the set of reachable
    points are identical. The set of reachable states is closed under merge, so
    so is the invariant, so the object is invariant closed.

    I know I'm walking through a lot of theory pretty quickly, but if you
