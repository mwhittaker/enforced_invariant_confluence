...
\begin{example}\examplelabel{SegmentedZ2}
  ...
  y)}{xy \leq 0}$ from \exampleref{Z2}, but now let the start state $s_0$ be
...
\defword{active segment}.  Initially, every server selects its active segment
...
\begin{example}
  ...
  where $t_{2\times}(x) = 2x$ and $t_{+10} = x + 10$. The states $3$ and $16$
  are \TIcoreachable{} because both are \stireachable{s}{T}{I} from $s = 3$
  ($16 = 2 \times 3 + 10$). The states $3$ and $5$, however, are not
...
leverage well-established means of ensuring fault tolerance without difficulty.
For example, we can replicate every server using a state machine replication
protocol like MultiPaxos~\cite{lamport1998part, lamport2001paxos} or
