...
\begin{revisions}
  ...
  s_3 = (p_1, p_2, p_3), (n_1, n_2, n_3) = (3, 4, 5), (3, 3, 3)$. Here, the
  value of the PN-counter is $(3 + 4 + 5) - (3 + 3 + 3) = 3$, which is
  ...
  seems safe. Server 1 would transition to state $(3, 3, 5), (3, 3, 3)$ which
  ...
  execute it locally and transition to state $(3, 3, 5), (3, 3, 3)$. This is
...
\begin{revisions}
  ...
  particularly well. Assume that $k = 3$ and that the three states are $s_1 =
  s_2 = s_3 = (p_1, p_2, p_3), (n_1, n_2, n_3) = (3, 3, 3), (3, 3, 3)$. Here,
  no server can process a decrement transaction locally. If a server receives a
  decrement transaction, it must execute it with global coordination. However,
  if the server instead defers the execution of the decrement transaction and
  happens to receive a number of increment transactions, then it can execute
  the decrement locally, without the need for global coordination.
\end{revisions}
