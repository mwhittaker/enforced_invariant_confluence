...
\begin{revisions}
  ...
  particularly well. Assume that $k = 3$ and that the three states are $(3, 3,
  3), (3, 3, 3)$. Here, no server can process a decrement transaction locally.
  If a server receives a decrement transaction, it must execute it with global
  coordination. However, if the server instead defers the execution of the
  decrement transaction and happens to receive a number of increment
  transactions, then it can execute the decrement locally, without the need for
  global coordination.
\end{revisions}
