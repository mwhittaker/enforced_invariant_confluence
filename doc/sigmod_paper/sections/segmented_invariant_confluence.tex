...
clear in the next subsection. $O$ is \defword{segmented \invariantconfluent{}}
...
%
Transaction execution in the new system model, on the other hand, is more
involved. Consider a server $s_i$ with active segment $(I_i, T_i)$. A client
can request that $p_i$ execute a transaction $t$. We consider what happens when
$t \in T_i$ and when $t \notin T_i$.
...
\in I$ are \defword{coreachable} with respect to a set of transactions $T$ and
an invariant $I$, abbreviated \defword{\TIcoreachable}, if there exists some
state $s_0 \in I$ such that $s_1$ and $s_2$ are both \sTIreachable{}.
...
simply invoke \algoref{ArbitraryStartInteractiveDecisionProcedure} once on each
segment.
...
\begin{example}\examplelabel{CounreachableExample}
  ...
  n_3$. After this, \IsIclosed{} (and thus \textsc{Helper}) returns true, and
  \algoref{ArbitraryStartInteractiveDecisionProcedure} terminates.
\end{example}
