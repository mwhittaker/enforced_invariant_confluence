...
then $s_1$ and $s_2$ are null. Otherwise, $s_1$ and $s_2$ are a counterexample
...
If $s_1$ or $s_2$ is in $R$, then they are reachable.
%
Second, we can prompt the user to tell us directly whether the states
...
\figref{DecisionProcedureExample} for example). In this step, we also make
...
invariant. See \figref{DecisionProcedureExample}, for example. Once $NR$ has
been sufficiently augmented to the point that $I - NR$ is a subset of the
reachable states, \thmref{IclosureEquivalentIconfluence} guarantees that the
algorithm will terminate after one more call to \IsIclosed.
