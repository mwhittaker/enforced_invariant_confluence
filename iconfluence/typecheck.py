...
def _assert_type_eq(typ: ast.Type,
                    expected: ast.Type,
                    sube: ast.Expr,
                    e: ast.Expr) \
                    -> None:
...
def _assert_type_uniform(typ_a: ast.Type,
                         typ_b: ast.Type,
                         sub_a: ast.Expr,
                         sub_b: ast.Expr,
                         e: ast.Expr) \
...
def _assert_type_instance(typ: ast.Type,
                          expected: type,
                          sube: ast.Expr,
                          e: ast.Expr) \
                          -> None:
...
def _typecheck_expr(e: ast.Expr, env: Dict[str, ast.Type]) -> ast.Expr:
    if isinstance(e, ast.EVar):
        ...
        typ = _typecheck_expr(e.x, env).typ
        _assert_type_instance(typ, ast.TTuple2, e.x, e)
        ...
        typ = _typecheck_expr(e.x, env).typ
        _assert_type_instance(typ, ast.TOption, e.x, e)
        e.typ = ast.TBool()
        ...
        _assert_type_eq(_typecheck_expr(e.lhs, env).typ, ast.TInt(), e.lhs, e)
        _assert_type_eq(_typecheck_expr(e.rhs, env).typ, ast.TInt(), e.rhs, e)
        e.typ = ast.TInt()
    elif (isinstance(e, ast.EBoolOr) or
          isinstance(e, ast.EBoolAnd) or
          isinstance(e, ast.EBoolImpl)):
        _assert_type_eq(_typecheck_expr(e.lhs, env).typ, ast.TBool(), e.lhs, e)
        _assert_type_eq(_typecheck_expr(e.rhs, env).typ, ast.TBool(), e.rhs, e)
        e.typ = ast.TBool()
        ...
        _assert_type_eq(rhs_typ, lhs_typ.a, e.rhs, e)
        e.typ = ast.TBool()
    elif isinstance(e, ast.EMapContainsKey):
        lhs_typ = _typecheck_expr(e.lhs, env).typ
        _assert_type_instance(lhs_typ, ast.TMap, e.lhs, e)
        ...
        lhs_typ = _typecheck_expr(e.lhs, env).typ
        rhs_typ = _typecheck_expr(e.rhs, env).typ
        _assert_type_uniform(lhs_typ, rhs_typ, e.lhs, e.rhs, e)
        e.typ = ast.TBool()
        ...
        _assert_type_instance(map_typ, ast.TMap, e.a, e)
        map_typ = cast(ast.TMap, map_typ)
        k_typ = _typecheck_expr(e.b, env).typ
        _assert_type_eq(k_typ, map_typ.a, e.b, e)
        v_typ = _typecheck_expr(e.c, env).typ
        _assert_type_eq(v_typ, map_typ.b, e.c, e)
        e.typ = map_typ
        ...
        _assert_type_eq(b_typ, ast.TBool(), e.a, e)
        _assert_type_uniform(if_typ, else_typ, e.b, e.c, e)
        e.typ = if_typ
    else:
